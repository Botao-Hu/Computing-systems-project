...
ThreadContext *__sthread_scheduler(ThreadContext *context) {
    ...
    /* OVERALL EXPLANATION FOR LOCK/UNLOCK MODIFICATION (PROBLEM 3)
     *
     * From the assembly code glue.s we can see that the program enters the
     * scheduler when either __sthread_start or __sthread_switch is called,
     * and they both result in calling __sthread_scheduler. So my strategy
     * is to do lock in every function which will lead to either 
     * __sthread_start or __sthread_switch:
     *
     * 1. sthread_start
     * 2. __sthread_finish
     * 3. sthread_yeild
     * 4. sthread_block.
     *
     * And do unlock once in __sthread_scheduler as the only leaving point.
     */
    __sthread_unlock();
...
void sthread_start(int timer) {
    ...
    /* Lock the thread since it will enter the scheduler in __sthread_start */
    __sthread_lock();
...
void __sthread_finish(void) {
    ...
    /* Lock the thread since it will enter the scheduler in __sthread_switch */
    __sthread_lock();
