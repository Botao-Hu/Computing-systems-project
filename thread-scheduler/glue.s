...
__sthread_lock:
        ...
        # The bts instruction selects the bit in the second argument at the 
        # bit-position designated by the offset in the first argument, stores
        # the value of the bit in the CF flag, and sets the selected bit in 
        # the first argument to 1. The second argument can either be a memory
        # address or a register. Since scheduler_lock is a 32-bit memory, the
        # first argument should be 0, specifying the lowest bit.
        # More importantly, this instruction can be used with a LOCK prefix to
        # allow the instruction to be executed atomically.
        lock bts    $0, scheduler_lock        

        # If CF = 1, which means scheduler_lock is 1, return 0 for lock.
        # Otherwise return 1 for available lock.
        jc          __lock_unavailable
        movl        $1, %eax
        ret
__lock_unavailable:
        movl        $0, %eax
...
__sthread_unlock:
        ...
        # Since manual states that writing to a double word aligned on a 32
        # bit boundary will ALWAYS be atomic, we can implement unlock with a
        # simple movl instruction.
